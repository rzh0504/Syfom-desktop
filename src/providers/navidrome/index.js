...
function parseSongListResponse(response) {
...
function parseAlbumListResponse(response) {
...
export async function updatePlaylistTracks({ pid, op, tracks }) {
...
export async function getArtistAlbums(id, limit = 200) {
  const response = await requestSubsonic('getArtist', { id });
...
export async function searchAll({ keywords, type, limit = 30, offset = 0 }) {
  if (!keywords) {
    return { result: {} };
  }
...
export async function getLibrarySongs({ offset = 0, limit = 100 } = {}) {
...
export async function starSong(id, like = true) {
  if (like) {
...
export async function starAlbum(id, like = true) {
  if (like) {
...
export async function starArtist(id, like = true) {
  if (like) {
